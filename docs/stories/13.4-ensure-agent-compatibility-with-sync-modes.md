...
- [ ] Task 1: Implement agent request identification (AC: 1, 2)
  - [ ] Add middleware to detect agent requests via API key or header
  - [ ] Implement sync mode parameter support in sanitization endpoints
  - [ ] Update endpoint logic to bypass async queue for sync requests
- [ ] Task 2: Ensure synchronous processing for agent operations (AC: 3, 4, 5)
  - [ ] Modify /api/sanitize and /api/sanitize/json to support sync mode
  - [ ] Update /api/documents/upload for sync PDF processing when requested
  - [ ] Verify trust token endpoints remain synchronous
- [ ] Task 3: Integration and performance testing (AC: 6, 7)
  - [ ] Test agent proxy flows with sync modes enabled
  - [ ] Performance benchmark agent sanitization operations
  - [ ] Validate no regression in agent monitoring and alerting
