...
def get_prometheus_metrics() -> Dict[str, Any]:
    ...
    try:
        ...
        return {
            ...
            "chat_requests_total": {"success": 89, "error": 3, "rate_limited": 1},
            ...
            gauge={
                ...
                "bar": {
                    "color": (
                        "green"
                        if success_rate > 95
                        else "orange" if success_rate > 85 else "red"
                    )
                },
                "steps": [
            ...
            gauge={
                ...
                "bar": {
                    "color": (
                        "red"
                        if cpu_usage > 90
                        else "orange" if cpu_usage > 70 else "green"
                    )
                },
            },
            ...
            gauge={
                ...
                "bar": {
                    "color": (
                        "red"
                        if memory_usage > 90
                        else "orange" if memory_usage > 70 else "green"
                    )
                },
            },
...
def main():
    ...
    st.markdown(
        "Real-time performance monitoring for PDF processing and system metrics"
    )
    ...
    with tab1:
        pdf_metrics = metrics.get("pdf_processing_requests_total", {})
        df_pdf = pd.DataFrame(
            {"Status": list(pdf_metrics.keys()), "Count": list(pdf_metrics.values())}
        )
        st.dataframe(df_pdf)
    ...
    with tab2:
        ...
        df_chat = pd.DataFrame(
            {"Status": list(chat_metrics.keys()), "Count": list(chat_metrics.values())}
        )
    ...
    with tab3:
        system_metrics = {
            ...
            "Active WebSocket Connections": metrics.get(
                "active_websocket_connections", 0
            ),
        }
        df_system = pd.DataFrame(
            {
                "Metric": list(system_metrics.keys()),
                "Value": list(system_metrics.values()),
            }
        )
