...
def detect_url(content: str) -> list[PIIMatch]:
    ...
    for match in re.finditer(bare_pattern, content):
        ...
        if any(
            m["start"] <= start < m["end"] or m["start"] < end <= m["end"]
            for m in matches
        ):
            continue
...
def _apply_mask_strategy(content: str, matches: list[PIIMatch]) -> str:
    ...
    for match in sorted(matches, key=lambda item: item["start"], reverse=True):
        ...
        if pii_type == "email":
            ...
            if separator:
                masked = (
                    f"****{separator}****{separator}****{separator}{digits_only[-4:]}"
                )
            else:
            ...
            masked = f"**{separator}**{separator}**{separator}**{separator}**{separator}{value[-2:]}"
        elif pii_type == "url":
            masked = "[MASKED_URL]"
        else:
