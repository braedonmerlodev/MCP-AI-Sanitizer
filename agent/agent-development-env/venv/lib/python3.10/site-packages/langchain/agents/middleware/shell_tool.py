    ...
    * `DockerExecutionPolicy` – launches a separate Docker container for each agent run,
    ...
    def _normalize_shell_command(
        ...
        if shell_command is None:
            return ("/bin/bash",)
        normalized = (
            (shell_command,) if isinstance(shell_command, str) else tuple(shell_command)
        )
        if not normalized:
    ...
    def before_agent(
        self, state: ShellToolState, runtime: Runtime
    ...
    def _create_resources(self) -> _SessionResources:
        ...
        return _SessionResources(
            session=session, tempdir=tempdir, policy=self._execution_policy
        )
    ...
    def _run_startup_commands(self, session: ShellSession) -> None:
        ...
        for command in self._startup_commands:
            result = session.execute(
                command, timeout=self._execution_policy.startup_timeout
            )
    ...
    def _run_shutdown_commands(self, session: ShellSession) -> None:
        ...
        for command in self._shutdown_commands:
            try:
                result = session.execute(
                    command, timeout=self._execution_policy.command_timeout
                )
                if result.timed_out:
                    LOGGER.warning("Shutdown command '%s' timed out.", command)
                elif result.exit_code not in (0, None):
                    LOGGER.warning(
                        "Shutdown command '%s' exited with %s.",
                        command,
                        result.exit_code,
                    )
            except (RuntimeError, ToolException, OSError) as exc:
                LOGGER.warning(
                    "Failed to run shutdown command '%s': %s",
                    command,
                    exc,
                    exc_info=True,
                )
        ...
        *,
        ...
        if payload.get("restart"):
            ...
            try:
                ...
                LOGGER.exception(
                    "Restarting shell session failed; session remains unavailable."
                )
        ...
        try:
            ...
            LOGGER.warning(
                "Blocking command output due to detected %s.", error.pii_type
            )
        ...
        if (
            result.truncated_by_bytes
            and self._execution_policy.max_output_bytes is not None
        ):
            sanitized_output = (
                f"{sanitized_output.rstrip()}\n\n"
