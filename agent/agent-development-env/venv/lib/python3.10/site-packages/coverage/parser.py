...
class PythonParser:
    ...
    def lines_matching(self, regex: str) -> set[TLineNo]:
        ...
        for match in re.finditer(regex, self.text, flags=re.MULTILINE):
            ...
            matches.update(
                self.multiline_map.get(i, i)
                for i in range(start_line + 1, end_line + 2)
            )
            last_start = start
    ...
    def _analyze_ast(self) -> None:
        ...
        assert self._ast_root is not None
        aaa = AstArcAnalyzer(
            self.filename, self._ast_root, self.raw_statements, self.multiline_map
        )
        aaa.analyze()
        arcs = aaa.arcs
...
class AstArcAnalyzer:
    ...
    ) -> TLineNo:  # pylint: disable=unused-argument
        return 1
