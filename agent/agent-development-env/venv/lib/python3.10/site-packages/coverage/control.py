...
class Coverage(TConfigurable):
    ...
    def __init__(  # pylint: disable=too-many-arguments
        ...
        if isinstance(config_file, str) and config_file.startswith(CONFIG_DATA_PREFIX):
            self.config = CoverageConfig.deserialize(
                config_file[len(CONFIG_DATA_PREFIX) :]
            )
        else:
    ...
    def _write_startup_debug(self) -> None:
        ...
        with self._debug.without_callers():
            if self._debug.should("config"):
                write_formatted_info(
                    self._debug.write, "config", self.config.debug_info()
                )
                wrote_any = True
            ...
            if self._debug.should("sqlite"):
                write_formatted_info(
                    self._debug.write, "sqlite", CoverageData.sys_info()
                )
                wrote_any = True
