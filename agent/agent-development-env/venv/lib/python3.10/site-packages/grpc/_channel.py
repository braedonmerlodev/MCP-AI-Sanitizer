                ...
                if state.code is None and not state.cancelled:
                    ...
                    def _done():
                        return (
                            ...
                            or cygrpc.OperationType.send_message not in state.due
                        )
...
def _stream_unary_invocation_operations(
    ...
    return (
        (
            cygrpc.SendInitialMetadataOperation(metadata, initial_metadata_flags),
            cygrpc.ReceiveMessageOperation(_EMPTY_FLAGS),
...
def _poll_connectivity(
    ...
    with state.lock:
        state.connectivity = _common.CYGRPC_CONNECTIVITY_STATE_TO_CHANNEL_CONNECTIVITY[
            connectivity
        ]
        callbacks = tuple(
...
def _subscribe(
    ...
    with state.lock:
        if not state.callbacks_and_connectivities and not state.polling:
            ...
            state.callbacks_and_connectivities.append([callback, state.connectivity])
        else:
            state.try_to_connect |= bool(try_to_connect)
            state.callbacks_and_connectivities.append([callback, None])
