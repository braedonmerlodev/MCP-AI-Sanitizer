...
class DaskNamespace(
    ...
    def concat(
        ...
        if how == "vertical":
            for i, df in enumerate(dfs[1:], start=1):
                ...
                if not (
                    (len(cols_current) == len(cols_0))
                    and (cols_current == cols_0).all()
                ):
                    msg = (
    ...
    def mean_horizontal(self, *exprs: DaskExpr) -> DaskExpr:
        def func(df: DaskLazyFrame) -> list[dx.Series]:
            ...
            series = align_series_full_broadcast(
                df, *(s.fillna(0) for s in expr_results)
            )
            ...
            den = reduce(
                lambda x, y: x + y, non_na
            )  # pyright: ignore[reportOperatorIssue]
            return [
                cast("dx.Series", num / den)
            ]  # pyright: ignore[reportOperatorIssue]
    ...
    def concat_str(
        ...
        def func(df: DaskLazyFrame) -> list[dx.Series]:
            ...
            null_mask = [
                s.isna() for s in align_series_full_broadcast(df, *expr_results)
            ]
    ...
    def when_then(
        ...
        def func(df: DaskLazyFrame) -> list[dx.Series]:
            ...
            return [
                then_series.where(condition, otherwise_series)
            ]  # pyright: ignore[reportArgumentType]
