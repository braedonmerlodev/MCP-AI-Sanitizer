...
def native_to_narwhals_dtype(
    ibis_dtype: IbisDataType, version: Version
) -> DType:  # noqa: C901, PLR0912
...
def narwhals_to_native_dtype(dtype: IntoDType, version: Version) -> IbisDataType:
    ...
    if isinstance_or_issubclass(dtype, dtypes.Datetime):
        return ibis_dtypes.Timestamp.from_unit(
            dtype.time_unit, timezone=dtype.time_zone
        )
    if isinstance_or_issubclass(dtype, dtypes.Duration):
        return ibis_dtypes.Interval(
...
def timedelta_to_ibis_interval(
    td: timedelta,
) -> ibis.expr.types.temporal.IntervalScalar:
