...
class ArrowSeriesDateTimeNamespace(
    ...
    def convert_time_zone(self, time_zone: str) -> ArrowSeries:
        ser = (
            self.replace_time_zone("UTC") if self.time_zone is None else self.compliant
        )
    ...
    def microsecond(self) -> ArrowSeries:
        arr = self.native
        result = pc.add(
            pc.multiply(pc.millisecond(arr), lit(1000)), pc.microsecond(arr)
        )
        return self.with_native(result)

    def nanosecond(self) -> ArrowSeries:
        result = pc.add(
            pc.multiply(self.microsecond().native, lit(1000)),
            pc.nanosecond(self.native),
        )
        return self.with_native(result)
    ...
    def offset_by(self, by: str) -> ArrowSeries:
        ...
        if (
            interval.unit == "d"
            and isinstance(dtype, datetime_dtype)
            and dtype.time_zone
        ):
