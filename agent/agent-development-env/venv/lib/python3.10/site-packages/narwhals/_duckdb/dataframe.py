...
class DuckDBLazyFrame(
    ...
    @staticmethod
    def _is_native(
        obj: duckdb.DuckDBPyRelation | Any,
    ) -> TypeIs[duckdb.DuckDBPyRelation]:
    ...
    def unique(
        ...
        *,
        ...
        flags = (
            extend_bool(True, len(order_by)) if order_by and keep == "last" else None
        )
        if keep == "none":
    ...
    def explode(self, columns: Sequence[str]) -> Self:
        ...
        zero = lit(0)
        not_null_condition = (
            col_to_explode.isnotnull() & F("len", col_to_explode) > zero
        )
